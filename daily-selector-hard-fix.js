/* ACROW Factory 5 — v227: last five daily-production machines stay independently selected */
(function(){
'use strict';
var KEY='acrow_daily_favorites_override_v227';
...
window.addEventListener('click',function(e){
 var cb=findCb(e.target);if(cb){var id=getId(cb);if(isFive(id)){e.preventDefault();e.stopImmediatePropagation();var a=base(),next=a.indexOf(id)<0;if(next)a.push(id);else a=a.filter(function(x){return x!==id;});storeSet(a);cb.checked=next;setTimeout(enforce,0);setTimeout(enforce,100);setTimeout(enforce,300);return;}}
 var b=e.target&&e.target.closest?e.target.closest('button'):null;
 if(b&&sid(b.textContent)==='تم'){
   setTimeout(function(){storeSet(getFiveFromDom());setTimeout(enforce,50);},0);
   setTimeout(function(){storeSet(getFiveFromDom());setTimeout(enforce,50);},250);
 }
},true);
...
function hook(){try{if(typeof window.renderMachineSelectList!=='function'||window.renderMachineSelectList.__acrow227)return;var old=window.renderMachineSelectList;var wrapped=function(){var r=old.apply(this,arguments);setTimeout(enforce,0);setTimeout(enforce,80);return r;};wrapped.__acrow227=true;window.renderMachineSelectList=wrapped;}catch(e){}}
