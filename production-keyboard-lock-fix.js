/* ACROW Factory 5 — mobile production keyboard hard lock */
(function(){
'use strict';
var machine='';
var value='';
...
function style(){
 ...
 s.textContent='.actual-input{touch-action:manipulation!important;-webkit-tap-highlight-color:transparent!important;}' +
 '.actual-input:focus{scroll-margin-bottom:260px!important;}' +
 'body.acrow-prod-keyboard-lock{overflow-anchor:none!important;}';
...
function burst(){
 if(!machine)return;
 [0,30,80,150,250,400,650,900,1300,1800].forEach(function(ms){
  setTimeout(function(){keep();},ms);
 });
}
...
function boot(){
 ...
 document.addEventListener('focusin',function(e){
  var el=e.target&&e.target.closest?e.target.closest('.actual-input'):null;
  if(!el)return;
  remember(el); burst();
 },true);
 document.addEventListener('input',function(e){
  var el=e.target&&e.target.closest?e.target.closest('.actual-input'):null;
  if(!el)return;
  remember(el); value=String(el.value||''); burst();
 },true);
 ...
 if(window.MutationObserver){
  new MutationObserver(function(){if(machine)burst();}).observe(document.body,{childList:true,subtree:true});
 }
