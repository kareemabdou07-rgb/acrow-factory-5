...
.actual-input{pointer-events:auto!important;touch-action:auto!important;position:relative!important;z-index:20!important;}
...
function decorate(){style();document.querySelectorAll('.actual-input').forEach(function(input){var row=input.closest('.mc-row')||input.parentElement;if(!row)return;row.classList.add('acrow-production-row');var id=String(input.dataset.machine||'').trim();if(!id)return;var b=row.querySelector('.'+BTN);if(!b){b=document.createElement('button');b.type='button';b.className=BTN;b.textContent='تثبيت';row.appendChild(b)}var r=rec(id);if(r&&r.actual!==null&&r.actual!==undefined&&document.activeElement!==input&&input.value!==String(r.actual))input.value=r.actual;paint(input,b,r)})}
...
var t;function watch(){clearTimeout(t);t=setTimeout(decorate,500)}
