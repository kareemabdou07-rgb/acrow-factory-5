...
function mergeMachineAndPlanData(remote,local){
  ...
  out.favorites=Array.from(new Set(rf.concat(lf).map(function(x){return String(x);}))).filter(Boolean);
  return out;
}
function mergeFirst(remote,local){
  var out=mergeMachineAndPlanData(remote,local);if(!out)return local||{};
  if(local&&local.records&&typeof local.records==='object'){out.records=Object.assign({},remote.records||{});Object.keys(local.records).forEach(function(k){if(!Object.prototype.hasOwnProperty.call(out.records,k))out.records[k]=local.records[k];});}
  return out;
}
...
function start(){if(!window.firebase||!firebase.initializeApp||!firebase.firestore){console.error('Firebase libraries unavailable');return;}try{if(!firebase.apps.length)firebase.initializeApp(cfg);var db=firebase.firestore(),ref=db.collection(COLLECTION).doc(DOC);window.__acrowFirebaseRef=ref;ready=true;ref.onSnapshot(function(snap){var remote=snap.exists&&snap.data()?snap.data().data:null,local=localStore();if(!remote){if(local){lastSent=js(local);ref.set({data:local,updatedAt:firebase.firestore.FieldValue.serverTimestamp()}).catch(console.error);}firstSnapshot=false;return;}var rj=js(remote);if(rj===lastSent||rj===lastRemote){firstSnapshot=false;return;}var focus=focusedProductionSnapshot(local);applyingRemote=true;try{var merged=firstSnapshot?mergeFirst(remote,local):mergeMachineAndPlanData(remote,local);store=merged;restoreFocusedProduction(focus);saveLocal();lastRemote=js(merged);if(firstSnapshot&&lastRemote!==rj){lastSent=lastRemote;ref.set({data:merged,updatedAt:firebase.firestore.FieldValue.serverTimestamp()}).catch(console.error);}firstSnapshot=false;if(!focus)renderAll();}finally{applyingRemote=false;}if(lastRemote!==lastSent)queue();},function(e){console.error('Firebase listener failed',e);});setInterval(function(){var s=localStore();if(!s||applyingRemote)return;var now=js(s);if(now!==lastSent)queue();},300);window.addEventListener('beforeunload',function(){try{cloudSave();}catch(e){}});console.log('ACROW Factory 5 Firebase realtime connected');}catch(e){console.error('Firebase startup failed',e);}}
