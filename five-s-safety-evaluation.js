...
function machines(){
  ...
  document.querySelectorAll('.machine-card').forEach(function(card){
    var id=String(card.dataset.machine||'').trim();
    if(!id||seen[id])return;
    seen[id]=1;
    var nameEl=card.querySelector('.mc-name');
    var name=nameEl?nameEl.textContent:id;
    out.push({id:id,name:String(name).replace(/\s+/g,' ').trim()});
  });
  if(out.length)return out;
  if(typeof MACHINES!=='undefined'&&Array.isArray(MACHINES)){
    MACHINES.forEach(function(m){var id=String(m.id||'').trim();if(!id||seen[id])return;seen[id]=1;out.push({id:id,name:String((typeof machineDisplayName==='function'?machineDisplayName(m):m.name)||id).replace(/\s+/g,' ').trim()});});
  }
  return out;
}
function target(){
  var h=document.querySelectorAll('.dept-header h2');
  for(var i=0;i<h.length;i++){var t=(h[i].textContent||'').trim();if(/إنتاج/.test(t)){return h[i].closest('.dept-section')||h[i].parentElement;}}
  var grid=document.querySelector('.machine-grid');
  if(grid)return grid.parentElement;
