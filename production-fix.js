...
document.addEventListener('input',function(e){var input=e.target&&e.target.closest?e.target.closest('.actual-input'):null;if(!input)return;e.stopImmediatePropagation();var id=String(input.dataset.machine||'').trim(),r=rec(id);if(r){r.actual=input.value===''?null:Number(input.value);r.productionFixed=false;try{saveStore()}catch(x){}}paint(input,input.closest('.mc-row')?.querySelector('.'+BTN),r)},true);
