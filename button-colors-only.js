...
/* v15 — stronger blue idle + bright yellow active */
(function(){
'use strict';
var activeButton=null;
var base='linear-gradient(135deg,#022f63 0%,#075aa3 55%,#1478bd 100%)';
...
function paint(b,active){if(!b)return;b.setAttribute('data-acrow-main-color','1');b.style.setProperty('background-image',active?'linear-gradient(135deg,#075aa3 0%,#1591c9 42%,#ffd21f 100%)':base,'important');b.style.setProperty('background-color','#075aa3','important');b.style.setProperty('color','#fff','important');b.style.setProperty('border','2px solid #b5a82f','important');b.style.setProperty('font-weight','1000','important');b.style.setProperty('font-size','1.09em','important');b.style.setProperty('text-shadow','0 1px 2px rgba(0,0,0,.38)','important');b.style.setProperty('letter-spacing','.15px','important');b.style.setProperty('transition','filter .08s ease,transform .08s ease,box-shadow .08s ease,background-image .08s ease','important');if(active){b.style.setProperty('box-shadow','0 0 0 3px rgba(255,214,25,.95),0 6px 18px rgba(0,55,120,.4)','important');b.style.setProperty('filter','brightness(1.12) saturate(1.1)','important');}else{b.style.removeProperty('box-shadow');b.style.removeProperty('filter');}}
