/* ACROW Factory 5 — authoritative daily machine selector sync */
(function(){
'use strict';
...
function add(map,order,m){if(!m)return;var id=sid(m.id||m.number||m.code||m.machine);if(!id)return;var name=String(m.name||m.type||m.machineName||('ماكينة '+id));if(!map[id]){map[id]={id:id,name:name};order.push(id);}else if(name)map[id].name=name;for(var k in m){if(m[k]!==undefined&&m[k]!==null)map[id][k]=m[k];}}
function currentMachines(){
 ...
 if(!order.length){try{if(Array.isArray(window.MACHINES))window.MACHINES.forEach(function(m){add(map,order,m);});}catch(e){}}
...
function syncDom(){
 ...
 rows.forEach(function(row){var id=rowId(row);if(!id)return;if(!allowed[id]){row.remove();return;}var m=allowed[id];var cb=row.querySelector('input[type="checkbox"]');if(cb)cb.setAttribute('data-machine',id);var spans=row.querySelectorAll('span');var label=spans.length?spans[spans.length-1]:null;if(label)label.textContent=m.name+' ('+id+')';});
...
setInterval(function(){syncWindowMachines();syncDom();},500);
})();
