...
.actual-input{pointer-events:auto!important;touch-action:auto!important;position:relative!important;z-index:20!important;-webkit-user-select:text!important;user-select:text!important;}
...
function decorate(){
 ...
 document.querySelectorAll('.actual-input').forEach(function(input){
  ...
  var r=rec(id);
  if(r&&r.actual!==null&&r.actual!==undefined&&document.activeElement!==input&&input.value!==String(r.actual))input.value=r.actual;
  paint(input,b,r);
 });
}
document.addEventListener('input',function(e){
 ...
 var id=String(input.dataset.machine||'').trim(),r=rec(id);
...
function wrapRender(){
 try{
  if(typeof window.render!=='function'||window.render.__acrowProductionWrapped)return;
  ...
  wrapped.__acrowProductionWrapped=true;
  window.render=wrapped;
 }catch(e){}
}
function start(){style();decorate();wrapRender();setTimeout(decorate,50);setTimeout(decorate,250);}
