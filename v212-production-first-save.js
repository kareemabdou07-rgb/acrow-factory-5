(function(){
  ...
  function mark(input,btn){
    if(input)input.classList.add('v212-production-saved');
    if(btn){
      btn.textContent='تم التثبيت';btn.classList.add('v212-saved');
      clearTimeout(timers.get(btn));
      timers.set(btn,setTimeout(function(){if(btn&&btn.isConnected){btn.textContent='تثبيت';btn.classList.remove('v212-saved');}},5000));
    }
  }
  function commit(input,btn){
    ...
    // Commit the exact value before the button causes blur/change.
    ...
    setTimeout(function(){
      ...
      cloudSave().then(function(){
        ...
        mark(input,btn);btn.dataset.busy='';
      }).catch(function(){
        try{delete document.body.dataset.v198RemotePending;}catch(e){}
        btn.dataset.busy='';
      });
  ...
  function bind(input){
    if(!isProd(input)||input.dataset.v212==='1')return;
    input.dataset.v212='1';
    input.addEventListener('input',function(){input.classList.remove('v212-production-saved');});
